...
from lab_2.bfs import BFSResult, BFSRunner
from lab_2.dfs import DFSResult, DFSRunner
from lab_2.data.default_graph import create_default_graph
from lab_2.graph import Edge, Graph, GraphError, Vertex


SearchResult = BFSResult | DFSResult
SearchRunner = BFSRunner | DFSRunner


class SearchApplication(ctk.CTk):
    def __init__(self) -> None:
        super().__init__()
        self.title("AI Systems Laboratory | BFS and DFS")
    ...
    def build_shell(self) -> None:
        ...
        create_label(self.sidebar, "Лабораторна робота №2", 11, color=COLORS["text_secondary"]).pack(padx=25, anchor="w")
        ...
        self.theory_button = create_button(self.sidebar, "Теорія BFS і DFS", self.show_theory, width=180, style="secondary")
    ...
    def show_lab(self) -> None:
        self.clear_main()
        create_title(self.main, "Лабораторна робота №2").pack(anchor="w")
        create_label(self.main, "Сліпий пошук на графах: BFS та DFS", 13, color=COLORS["text_secondary"]).pack(anchor="w", pady=(5, 15))
    ...
    def build_controls(self, parent) -> None:
        ...
        self.add_field(left, "Алгоритм пошуку")
        self.algorithm_var = ctk.StringVar(value="BFS")
        self.algorithm_combo = create_combobox(left, ["BFS", "DFS"], 275)
        self.algorithm_combo.configure(variable=self.algorithm_var, command=self.change_algorithm)
        self.algorithm_combo.pack(padx=18, pady=(3, 10))
        ...
        create_button(left, "Побудувати шлях без анімації", self.run_search_instant, width=260, style="success").pack(padx=10, pady=(5, 7))
    ...
    def build_visualization(self, parent) -> None:
        ...
        self.visualization_title = create_label(header, "Візуалізація BFS", 18, True)
        ...
        self.bind_all("<KeyPress-A>", self.add_keyboard_vertex)
        legend = create_label(right, "Shift + клік: вибір | F: неорієнтоване ребро | D: ребро light purple → purple | G: видалити | A: додати вершину | жовтий: DFS буде відвідано", 11, color=COLORS["text_secondary"])
    ...
    def change_algorithm(self, _value: str = "") -> None:
        self.reset_search()
        if hasattr(self, "visualization_title"):
            self.visualization_title.configure(text=f"Візуалізація {self.algorithm_var.get()}")

    def create_runner(self) -> SearchRunner:
        try:
            start, target = int(self.start_var.get()), int(self.target_var.get())
            custom = self.parse_custom_order()
        except ValueError as error:
            raise ValueError(str(error)) from error
        runner_class = DFSRunner if self.algorithm_var.get() == "DFS" else BFSRunner
        return runner_class(self.graph, start, target, self.order_var.get(), custom)
    ...
    def run_search(self) -> None:
        ...
        self.result_label.configure(text=f"{self.algorithm_var.get()} виконується...")
        self.animate_step()
    ...
    def draw_graph(self) -> None:
        ...
        future = set(self.runner.stack) if isinstance(self.runner, DFSRunner) else set()
        if isinstance(self.runner, DFSRunner):
            visited = visited - set(self.runner.stack)
        ...
        #         )
        # Формуємо множину саме тих ребер, які входять у знайдений шлях.
        # Для неорієнтованих ребер враховуємо обидва напрямки.
        path_edges = set()
        
        if self.last_result and self.last_result.path:
            path_vertices = self.last_result.path
            ...
            for i in range(len(path_vertices) - 1):
                ...
                # Якщо графове ребро неорієнтоване,
                # воно є тим самим ребром і в зворотному напрямку.
                path_edges.add((b, a))
        
        for edge in self.graph.edges:
            source, target = point(
                self.graph.vertices[edge.source]
            ), point(
                self.graph.vertices[edge.target]
            )
        
            # Підсвічуємо тільки те ребро, яке реально входить у path.
            is_path_edge = (edge.source, edge.target) in path_edges
        ...
        for vertex_id, vertex in self.graph.vertices.items():
            ...
            if self.selected_vertices:
                if len(self.selected_vertices) == 2 and vertex_id == self.selected_vertices[0]:
                    outline = "#c084fc"
                elif vertex_id == self.selected_vertices[-1]:
                    outline = "#7e22ce"
            selected = vertex_id in self.selected_vertices
            special_vertex = vertex_id in {int(self.start_var.get() or 0), int(self.target_var.get() or 0)}
            self.canvas.create_oval(x - radius, y - radius, x + radius, y + radius, fill=fill, outline=outline, width=3 if selected or special_vertex else 1)
    ...
    def select_vertex(self, vertex_id: int) -> None:
        ...
        if len(self.selected_vertices) == 2:
            first, second = self.selected_vertices
            self.status_label.configure(text=f"Вибрано ребро: {first} → {second}", text_color="#7e22ce")
        elif self.selected_vertices:
            self.status_label.configure(text=f"Вибрано вершину: {self.selected_vertices[0]}", text_color="#7e22ce")
        else:
    ...
    def selected_edge_vertices(self) -> tuple[int, int] | None:
        if len(self.selected_vertices) != 2:
            self.status_label.configure(text="Виберіть дві вершини через Shift + клік", text_color=COLORS["danger"])
            return None
        return self.selected_vertices[0], self.selected_vertices[1]

    def set_selected_edge(self, directed: bool) -> None:
        selected = self.selected_edge_vertices()
        if selected is None:
            return
        ...
        try:
            matching_index = next(
                (
                    index for index, edge in enumerate(self.graph.edges)
                    if {edge.source, edge.target} == {source, target}
                ),
                None,
            )
            if matching_index is None:
                self.graph.add_edge(source, target, directed)
            else:
                self.graph.edges[matching_index] = Edge(source, target, directed)
            self.refresh_after_edit(clear_selection=False)
            direction = f"{source} → {target}" if directed else f"{source} — {target}"
            self.status_label.configure(text=f"Створено ребро: {direction}", text_color=COLORS["success"])
        except GraphError as error:
            messagebox.showerror("Ребро", str(error))

    def make_undirected_selected_edge(self, _event=None) -> None:
        self.set_selected_edge(False)

    def make_directed_selected_edge(self, _event=None) -> None:
        self.set_selected_edge(True)

    def delete_selected_items(self, _event=None) -> None:
        if not self.selected_vertices:
            self.status_label.configure(text="Виберіть вершину або дві вершини через Shift + клік", text_color=COLORS["danger"])
            return
        try:
            if len(self.selected_vertices) == 1:
                vertex_id = self.selected_vertices[0]
                self.graph.remove_vertex(vertex_id)
                message = f"Вершину {vertex_id} видалено"
            else:
                source, target = self.selected_vertices
                if self.graph.graph_type == "tree":
                ...
                message = f"Ребра між {source} і {target} видалено"
            self.refresh_after_edit()
            self.status_label.configure(text=message, text_color=COLORS["success"])
        except GraphError as error:
            messagebox.showerror("Видалення", str(error))

    def add_keyboard_vertex(self, _event=None) -> None:
        vertex_id = 1
        while vertex_id in self.graph.vertices:
            vertex_id += 1
        try:
            self.graph.add_vertex(Vertex(vertex_id, 40, 40))
            self.refresh_after_edit()
            self.status_label.configure(text=f"Вершину {vertex_id} додано у верхньому лівому куті", text_color=COLORS["success"])
        except GraphError as error:
    ...
    def add_experiment(self) -> None:
        if not self.last_result:
            messagebox.showinfo("Експеримент", "Спочатку виконайте пошук.")
            return
        result = self.last_result
    ...
    def show_theory(self) -> None:
        self.clear_main()
        create_title(self.main, "Теорія: BFS і DFS").pack(anchor="w")
        card = create_card(self.main)
        card.pack(fill="both", expand=True, pady=(15, 0))
        text = (
            "BFS досліджує граф рівень за рівнем, використовуючи чергу FIFO.\n"
            "DFS заглиблюється якомога далі, використовуючи стек LIFO.\n\n"
            "Алгоритм:\n"
            ...
            "DFS може використовувати менше пам'яті та швидко знаходити ціль у глибокій гілці, "
            "але не гарантує найкоротший шлях.\n\n"
